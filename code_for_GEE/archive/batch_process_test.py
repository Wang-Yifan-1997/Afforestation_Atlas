"""
Batch process countries through Google Earth Engine
Test version: First 2 countries only
UPDATED: Controls inherit all IDs from treated polygons
"""
...
print("="*70)
print("GOOGLE EARTH ENGINE BATCH PROCESSOR - TEST (2 COUNTRIES)")
print("="*70)
...
print(f"✓ Will process {len(COUNTRIES)} countries (TEST)")
    ...
    def generate_controls_for_one(treated_feature):
        ...
        return control_points.map(lambda point: 
            ee.Feature(point.buffer(radius)).set({
                ...
                'country': country,
                # **COPY ALL IDs FROM TREATED POLYGON**
                'ctry_id': ctry_id,
                'proj_id': proj_id,
                'site_id': site_id,
                'site_rpt': site_rpt
...
def process_country(asset_name, country_name):
    ...
    try:
        ...
        task4 = ee.batch.Export.table.toDrive(
            ...
            fileFormat='CSV',
            # **UPDATED SELECTORS TO INCLUDE ALL IDs**
...
print("\n" + "="*70)
print("PROCESSING COUNTRIES (TEST: 2 COUNTRIES)")
print("="*70)
