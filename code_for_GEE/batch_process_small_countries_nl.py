"""
Batch process SELECTED African countries through Google Earth Engine
UPDATED: Includes nighttime lights, selected countries only
"""
...
print("="*70)
print("GOOGLE EARTH ENGINE BATCH PROCESSOR - SELECTED COUNTRIES")
print("="*70)
...
COUNTRIES = [
    ...
    {'asset': 'central_african_rep__treated_polygons', 'name': 'Central African Rep'},
    {'asset': 'chad_treated_polygons', 'name': 'Chad'},
    {'asset': 'congo_treated_polygons', 'name': 'Rep of the Congo'},
    {'asset': 'c_te_d_ivoire_treated_polygons', 'name': "Cote d'Ivoire"},
    {'asset': 'gabon_treated_polygons', 'name': 'Gabon'},
    {'asset': 'gambia_treated_polygons', 'name': 'Gambia, The'},
...
def extract_covariates(polygons, id_field, is_treated):
    """Extract Hansen, NDVI, and nighttime lights covariates for polygons"""
    ...
    # **NIGHTTIME LIGHTS**
    # VIIRS data available from 2012-present
    nightlights = ee.ImageCollection('NOAA/VIIRS/DNB/MONTHLY_V1/VCMSLCFG').select('avg_rad')
    ...
    # **NIGHTTIME LIGHTS (2012-2024)**
    def extract_nightlights_year(year):
        year = ee.Number(year)
        nl_year = nightlights.filter(
            ee.Filter.calendarRange(year, year, 'year')
        ).filterBounds(polygons.geometry()).mean()
        
        return nl_year.reduceRegions(
            collection=polygons,
            reducer=reducer,
            scale=500,
            tileScale=4
        ).map(lambda f: create_props(f, year, 'nightlights', id_field, is_treated))
    
    nightlights_data = ee.FeatureCollection(
        ee.List.sequence(2012, END_YEAR).map(extract_nightlights_year)
    ).flatten()
    
    return hansen_baseline.merge(hansen_loss).merge(hansen_gain).merge(landsat_ndvi).merge(modis_ndvi).merge(nightlights_data)
...
# =============================================================================
# STEP 4: PROCESS SELECTED COUNTRIES
# =============================================================================

print("\n" + "="*70)
print(f"PROCESSING {len(COUNTRIES)} SELECTED COUNTRIES")
print("="*70)
