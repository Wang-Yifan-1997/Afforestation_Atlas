...
- **Official Source**: [Geofabrik Africa Download Portal](https://download.geofabrik.de/africa.html)
...
- **Official Source**: [Google Open Buildings Download Colab Notebook](https://colab.research.google.com/github/google-research/google-research/blob/master/building_detection/open_buildings_download_region_polygons.ipynb#scrollTo=pm-hsiYIt7x2)
...
# -------------------------- VISUALIZATION (FIXED) --------------------------
import matplotlib.patches as mpatches  # Add this import at the top
import matplotlib.lines as mlines      # Add this import at the top
...
# Plot data WITHOUT label parameter (we'll use proxies instead)
gdf_roads_proj.plot(ax=ax, linewidth=0.3, color='gray', alpha=0.6)
gdf_polygons_proj.plot(ax=ax, facecolor='none', edgecolor='red', linewidth=1.5)

# --- Create proxy artists for legend ---
# Proxy for roads (Line2D, since roads are lines)
road_proxy = mlines.Line2D(
    [], [], color='gray', alpha=0.6, linewidth=0.3, label='Roads'
)
# Proxy for afforestation zones (Patch, since zones are polygons)
zone_proxy = mpatches.Patch(
    facecolor='none', edgecolor='red', linewidth=1.5, label='Afforestation Zones'
)

# Add legend using proxies
ax.legend(handles=[road_proxy, zone_proxy], loc='upper right')
...
# Plot 2: Average road length by planting year (no legend issue here)
